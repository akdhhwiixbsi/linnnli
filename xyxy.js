/*****
^https://api\.yingyutingting\.com/1\.1/users/me url script-response-body https://raw.githubusercontent.com/akdhhwiixbsi/linnnli/refs/heads/main/xyxy.js
^https://buy\.itunes\.apple\.com/verifyReceipt url script-response-body https://raw.githubusercontent.com/akdhhwiixbsi/linnnli/refs/heads/main/xyxy.js
[mitm] 
hostname = api.yingyutingting.com,buy.itunes.apple.com
**/
// 合并脚本：处理 api.yingyutingting.com 和 buy.itunes.apple.com 的响应修改
let url = $request.url;

if (url.indexOf('api.yingyutingting.com/1.1/users/me') !== -1) {
    // 第一个脚本逻辑：字符串替换，插入会员信息字段
    let body = $response.body;
    body = body.replace(/"clientType":"ios"/g, '"clientType":"ios", "pollykannProIAP":"vip.quarterly.pollykann\\n[2024-07-22 22:28:28-->2088-03-09 02:01:47]","pollykannIAPProPurchasedDate":{"__type":"Date","iso":"2024-07-22T14:28:28.000Z"}');
    $done({body});
} 
else if (url.indexOf('buy.itunes.apple.com/verifyReceipt') !== -1) {
    // 第二个脚本逻辑：完全替换为伪造的收据响应
    var objc = {
